...
const senderConfig = {
  user: {
    icon: User,
    label: "Customer",
    bubbleClass: "bg-chat-user text-foreground",
    align: "items-start",
  },
  bot: {
    icon: Bot,
    label: "Bot",
    bubbleClass: "bg-chat-bot text-foreground",
    align: "items-end",
  },
  agent: {
    icon: Headphones,
    label: "Agent",
    bubbleClass: "bg-chat-agent text-foreground",
    align: "items-end",
  },
};
...
export function ChatView({ conversation, messages }: Props) {
  ...
  const [isBotMode, setIsBotMode] = useState(
    (conversation as any).chat_mode === "bot"
  );
  ...
  // Sync bot mode when conversation changes
  useEffect(() => {
  ...
  const handleSuggest = async () => {
    ...
    try {
      ...
      setReply(suggestion);
      toast.success("AI suggestion loaded — edit or send as-is");
    } catch (err) {
  ...
  const handleClaim = () => {
    claimConversation.mutate(conversation.id, {
      ...
      onError: (err) =>
        toast.error("Failed to claim: " + (err as Error).message),
    });
  };
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <Hand
                className={`w-4 h-4 ${!isBotMode ? "text-primary" : "text-muted-foreground"}`}
              />
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex flex-col ${config.align}`}>
              ...
              <div
                className={`${config.bubbleClass} rounded-2xl px-4 py-2.5 max-w-[85%] md:max-w-[75%] text-sm leading-relaxed ${
      ...
      <div className="px-4 md:px-6 py-3 md:py-4 border-t border-border bg-card">
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            placeholder={
              isBotMode
                ? "AI is auto-replying... (type to override)"
                : "Type a reply..."
            }
